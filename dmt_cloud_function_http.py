...
def GenerateConfig(ctx):
  ...
  function_name = config.deployment + 'config-builder-cf'
  ...
  for imp in ctx.imports:
    if imp.startswith(ctx.properties['codeLocation']): 
      zip_file.writestr(imp[len(ctx.properties['codeLocation']):],
                        ctx.imports[imp])
  # used the files below to extract a copy of what parameter 'ctx' contains.  Would be nice to get the object seriallize
  # but I was unnsuccessfull in doing so.  TODO - nice to have
