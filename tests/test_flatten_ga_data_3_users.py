...
class TestCFFlattenMethodsUsers(BaseUnitTest):
    ...
    def test_flatten_ga_data_sharded(self):
        ...
        assert self.tbl_exists(dataset=self.ga_source.dataset,
                               table_name=f"flat_users_{self.ga_source.date_shard}")

        assert self.tbl_exists(dataset=self.ga_source.dataset,
                               table_name=f"flat_users_user_properties_{self.ga_source.date_shard}")
    ...
    def test_flatten_ga_data_partitioned(self):
        ...
        assert self.tbl_exists(dataset=self.ga_source.dataset,
                               table_name=f"flat_users")

class TestCFFlattenMethodsUsersSourceDoesNotExist(BaseUnitTest):
    c = Context()
    ga_source = GaExportedNestedDataStorage(gcp_project=c.env["project"],
                                            dataset="analytics_123456789",
                                            table_type="users",
